        ...
        include: [
            {
                ...
                attributes: ['streetAddress', 'province', 'postalCode', 'district', 'lat', 'lng', 'coordinates', 'addressType', 'zoneId'],
                include: [
                    {
                        model: countries,
                        attributes: ['name']
...
async function orderDetailsById(req, res) {
    ...
    if (bookingfind.bookingStatusId === 5) {
        ...
        return res.json(responsefunc("1", `Order Details for ${Object.keys(whereCondition)[0]}: ${Object.values(whereCondition)[0]}`, { bookingfind, oneHourLater }, ""));
    }

    return res.json(responsefunc("1", `Order Details for ${Object.keys(whereCondition)[0]}: ${Object.values(whereCondition)[0]}`, bookingfind, ""));
}
...
async function agentBookingFilters(req, res) {
    ...
    if (filterType === 'pick') {
        ...
        return res.json(responsefunc("1", `Booking Details Fetch for slots`, slotBookings, ""));
    }
    ...
    return res.json(responsefunc("1", `Booking Details Fetch on the basis of ${filterType}`, bookingFound, ""))
}
...
async function agentBookingStatusOnTheWay(req, res) {
    ...
    return res.json(responsefunc("1", "Booking Status Updated Sucesfully", {}, " "))
...
*/async function driverStatusArrived(req, res) {
    ...
    return res.json(responsefunc("1", "Booking Status Updated to Driver Arrived", {}, " "))
}
...
async function AddPickupDeliveryProof(req, res) {
    ...
    if (!req.files.length) {
        throw new customError("Proof Images are not uploaded", "Please Upload the Images");
    }
    ...
    return res.json(responsefunc("1", "Driver proof Pics Uploaded Successfully", {}, ""));
}
...
async function agentInspectionStatus(req, res) {
    ...
    return res.json(responsefunc("1", "Booking PickingUp and Inspection Status Updated", {}, ""))
}
...
async function laundryDeliverToCustomer(req, res) {
    ...
    return res.json(responsefunc("1", "Driver updated and out for Deliver Laundry to Customer", {}, ""))
...
async function bookingDeliverToCustomer(req, res) {
    ...
    return res.json(responsefunc("1", "Laundry Delivered to customer sucessfully", {}, ""))
    ...
    const invoiceDetails = await booking.findOne({
        ...
        include: [
            ...
            },
            {
                ...
                attributes: ['date', 'time', 'categoryPrice', 'bookingId', 'categoryId', 'serviceId', 'subCategoryId', 'items']
            },
            {
...
async function onHoldConformation(req, res) {
    const { serviceId, subCategoryId, bookingId, noOfItems, description } = req.body
    ...
    return res.json(responsefunc("1", "Hold Conformation Submitted", createConformation, ""))
...
async function agentIssueResolved(req, res) {
    ...
    return res.json(responsefunc("1", "Booking Status Updated Issue Resolved", {}, ""))
...
async function agnetDrivers(req, res) {
        ...
        include: [
            ...
            },
            {
                ...
                include: [{
                    ...
                    attributes: ['streetAddress', 'province', 'lat', 'lng', 'addressType']
                }]
            }
        ],
    ...
    return res.json(responsefunc("1", "All Drivers Fetched for this Laundry Shop", driverFound, " "))
